 ...
 * License: MIT
 *
 * Modified, patched by jordic@gmail.com
 *
...
var (
	configFile    = flag.String("config", "lanyon/lanyon.json", "specify a config cile")
	watch         = flag.Bool("watch", true, "template floder watch for reloads")
	render        = flag.Bool("render", false, "Render as static website")
...
func getRequest(w http.ResponseWriter, r *http.Request) {
	...
	if fi, err := os.Stat(fullpath); err == nil {
		if fi.IsDir() {
			//var page Page
			html, _, err = getDirectoryListing(fullpath)
			if err != nil {
				showFourOhFour(w, r)
				return
			}
		...
		case ".html":
			//var page Page
			html, _, err = getMarkdownFile(fullpath)
			if err != nil {
				showFourOhFour(w, r)
				return
			}
...
func getDirectoryListing(dir string) (html string, page Page, err error) {
...
func getMarkdownFile(fullpath string) (html string, page Page, err error) {
	...
	if _, err := os.Stat(mdfile); err == nil {
		...
		return "", page, fmt.Errorf("Error reading file %s ", mdfile)
	}
...
func showFourOhFour(w http.ResponseWriter, r *http.Request) {
	...
	html, _, err := getMarkdownFile(md404)
	if err != nil {
		http.NotFound(w, r)
		return
	}
